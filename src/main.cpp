...
#if defined(NDEBUG)
# error "FUTURETRENDCOIN cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2eef6defbc9abf716a42280445a4d680516f09159dd41ba18c283d20d0ef33bf");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FUTURETRENDCOIN: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "FUTURETRENDCOIN Signed Message:\n";
...
{
    // FUTURETRENDCOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // FUTURETRENDCOIN: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // FUTURETRENDCOIN: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1490797210, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "New coin with new technology";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 6000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04503ab259bf1701c693ba6b5927edca43d94bd551003d76e3dd9b8c10f3500e14c3b83d08aaaaecc867c0d7122b4cf7147505d3a00874b58272a9513fc3a12a93") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1490797210;
        ...
        assert(block.hashMerkleRoot == uint256("0x2766490303b7df4afdd3085876f7f114f9573ad3b69fa64378dfcbbfe0ab325c"));
...
unsigned char pchMessageStart[4] = { 0x82, 0x1c, 0x65, 0x71 }; // FUTURETRENDCOIN: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FUTURETRENDCOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FUTURETRENDCOINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FUTURETRENDCOINMiner(CWallet *pwallet)
{
    printf("FUTURETRENDCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("futuretrendcoin-miner");
    ...
    try { loop {
        ...
        printf("Running FUTURETRENDCOINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FUTURETRENDCOINMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FUTURETRENDCOINMiner, pwallet));
}
